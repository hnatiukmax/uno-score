package dev.hnatiuk.core.presentation.recyclerview.layoutmanager
...
class SpannableGridLayoutManager : RecyclerView.LayoutManager {
    ...
    private val minimumFirstVisibleRow: Int
        get() {
            val maxDisplayedRows = ceil((height / cellHeight).toDouble()).toInt() + 1
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        val mAttrs = context.obtainStyledAttributes(attrs, R.styleable.SpannableGridLayoutManager, defStyleAttr, defStyleRes)
        columns = mAttrs.getInt(R.styleable.SpannableGridLayoutManager_spanCount, 1)
        parseAspectRatio(mAttrs.getString(R.styleable.SpannableGridLayoutManager_aspectRatio))
        val orientation = mAttrs.getInt(R.styleable.SpannableGridLayoutManager_android_orientation, RecyclerView.VERTICAL)
        mAttrs.recycle()
    }
    ...
    override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        var availableSpace: Int = height - scrollOffset
    ...
    override fun scrollVerticallyBy(
        ...
        if (dy < 0) { // scrolling content down
            ...
            val lastRowTop: Int = getDecoratedTop(
            ...
            val bottom: Int = getDecoratedBottom(getChildAt(childCount - 1)!!)
            scrolled = if (lastVisiblePosition == itemCount - 1) { // is at end of content
                val scrollRange = max(bottom - height + paddingBottom, 0)
                min(dy, scrollRange)
            } else {
                dy
            }
    ...
    private fun calculateCellPositions(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        for (position in 0 until itemCount) {
            ...
            val adapterPosition: Int = recycler.convertPreLayoutPositionToPostLayout(position)
    ...
    private fun layoutRow(
        ...
        while (position <= lastPositionInRow) {
            ...
            val wSpec: Int = getChildMeasureSpec(
                cellBorders[cell.column + cell.columnSpan] - cellBorders[cell.column],
                View.MeasureSpec.EXACTLY, 0, lp.width, false
            )
            val hSpec: Int = getChildMeasureSpec(
                cell.rowSpan * cellHeight,
                View.MeasureSpec.EXACTLY, 0, lp.height, true
            )
            measureChildWithDecorationsAndMargin(view, wSpec, hSpec)
            val left: Int = cellBorders[cell.column] + lp.leftMargin
            val top: Int = startTop + cell.row * cellHeight + lp.topMargin
            val right: Int = left + getDecoratedMeasuredWidth(view)
            val bottom: Int = top + getDecoratedMeasuredHeight(view)
    ...
    private fun calculateCellBorders() {
        ...
        val totalSpace: Int = width - paddingLeft - paddingRight
        var consumedPixels: Int = paddingLeft
    ...
    private fun measureChildWithDecorationsAndMargin(child: View, widthSpec: Int, heightSpec: Int) {
        ...
        mWidthSpec = updateSpecWithExtra(
            mWidthSpec, lp.leftMargin + itemDecorationInsets.left,
            lp.rightMargin + itemDecorationInsets.right
        )
        mHeightSpec = updateSpecWithExtra(
            mHeightSpec, lp.topMargin + itemDecorationInsets.top,
    ...
    data class SpanInfo(var columnSpan: Int, var rowSpan: Int)
